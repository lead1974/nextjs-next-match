...
import { MemberMessageDto } from '@/types';
...
export default function MessageTable({ messages }: Props) {
    ...
    const handleDeleteMessage = useCallback(async (message: MemberMessageDto) => {
    ...
    const renderCell = useCallback((item: MemberMessageDto, columnKey: keyof MemberMessageDto) => {
                        ...
                        <TableRow key={item.id} className='cursor-pointer'>
                            {(columnKey) => (
                                <TableCell className={`${!item.dateRead && !isOutbox ? 'font-semibold' : ''}`}>
                                    {renderCell(item, columnKey as keyof MemberMessageDto)}
                                </TableCell>
                            )}
                        </TableRow>
                    )}
